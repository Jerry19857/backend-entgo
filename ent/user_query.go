...
func (uq *UserQuery) sqlAll(ctx context.Context) ([]*User, error) {
	...
	if query := uq.withImage; query != nil {
		...
		for _, n := range neighbors {
			fk := n.user_image
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "user_image" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "user_image" returned %v for node %v`, *fk, n.ID)
			}
			node.Edges.Image = append(node.Edges.Image, n)
		}
	}
